...
8. Önceki bağlam tüketilmeden yeni keşif başlatılmaz: `/recall`, `prompt-memory-mcp_search_memory` ve `codebase-memory-mcp_search_graph` çalıştırılarak mevcut breadcrumb'lar okunur (Section 38).
...
## 38. Agentic Breadcrumb Protocol (otomatik MCP breadcrumbs)

Bu bölüm Section 36'daki "açık talimat yoksa memory çağırma" kuralına bir **istisna**dır. Amaç, agent'ların keşif ve karar süreçlerinde kaybolmaması için her adımda "ekmek kırıntısı" bırakmaktır.

### 38.1 Zorunlu recall önceliği

Herhangi bir dosya okumadan, kod değiştirmeden veya implementasyona başlamadan önce agent şu sırayla önceki bağlamı tüketmek zorundadır:

1. `/recall <topic>` veya `prompt-memory-mcp_search_memory`.
2. `codebase-memory-mcp_search_graph` veya ilgili `query` ile sembol keşfi.
3. Önceki görevden kalan `handoff` Memory'leri oku.

Eğer recall sonucunda ilgili özet veya Decision varsa, tekrar aynı dosyalara baştan bakmadan önce bunları kullan.

### 38.2 Keşif sonrası otomatik memory yazma

Aşağıdaki MCP/tool çağrılarından hemen sonra agent kısa bir `Memory` veya `Snippet` node kaydetmelidir:
...
- `exa_web_search_exa` (sonuç önemliyse)

Memory içeriği şunları içermeli: `query_intent`, `tool_used`, `key_findings` (dosya/sembol/line), `tags`.

### 38.3 Graph trace geri besleme

`codebase-memory-mcp` ile keşfedilen caller/callee ilişkileri, `codebase-memory-mcp_ingest_traces` aracıyla knowledge graph'e geri yazılır. Böylece sonraki agent'lar graph üzerinden aynı path'leri tekrar keşfetmek zorunda kalmaz.

### 38.4 Karar ve milestone yazma

Aşağıdaki durumlarda `Decision` node kaydedilir:

- Bir implementasyon/edit kararı verildiğinde.
- Bir test stratejisi seçildiğinde.
- Bir hata kökeni bulunduğunda.
- Bir performance/security trade-off yapıldığında.

### 38.5 Task handoff

Bir agent görevi bitirmeden önce `<project>-handoff-<task>-<timestamp>` isimli bir `Memory` node yazar:

- Görev hedefi
- Üretilen sonuç
- Değiştirilen dosyalar
- Bilinen kısıtlar/blocker'lar
- Önerilen sonraki adım

Sonraki agent önce bu handoff'ları tüketir.

### 38.6 Subagent breadcrumbs

Bir agent alt-agent görevlendirdiğinde, alt-agent yukarıdaki protokolü uygular. Bitişte parent'e kısa handoff summary gönderir; parent bunu `prompt-memory` MCP'ye kaydeder.

### 38.7 Otomasyon seviyesi

- **Keşif araçları:** her çağrı sonrası otomatik summary (agresif).
- **Implementasyon/test/debug:** her logical milestone sonrası `Decision` node (pragmatik).
- Bu seviye, Section 36'yı ihlal etmez; Section 36'ya istisna olarak burada tanımlanmıştır.
